...
Compare DMLF twin boundary energy (gamma_Twin)
...
Outputs:
  - SFE_DMLF_Ising_Twin_comparison.csv
  - DMLF_vs_Ising_Twin_100K.png / _350K.png / _550K.png
"""
...
def load_sfe_data(csv_path: str = "SFE_results_AlCrCo.csv") -> pd.DataFrame:
    ...
    required = [
        ...
        "A_FCC_A2", "natoms", "gamma_ISF_DMLF_mJm2", "gamma_Twin_DMLF_mJm2"
    ]
    for c in required:
...
def compute_ising_gammas(df: pd.DataFrame) -> pd.DataFrame:
    """
    Same axial Ising construction as for ISF:

      gamma_ISF^Ising = 2 * (E_hcp - E_fcc) * natoms / A_fcc

    A simple twin boundary can be thought of as two back-to-back intrinsic faults,
    so we approximate:

      gamma_Twin^Ising = 2 * gamma_ISF^Ising

    Adds:
      - gamma_ISF_Ising_mJm2
      - gamma_Twin_Ising_mJm2
    """
    df = df.copy()
    ...
    df["gamma_Twin_Ising_mJm2"] = 2.0 * df["gamma_ISF_Ising_mJm2"]
...
def plot_dmlf_vs_ising(df: pd.DataFrame,
                       color_map: dict,
                       out_prefix: str = "DMLF_vs_Ising_Twin"):
    temps = sorted(df["Temp_K"].dropna().unique())

    for T in temps:
        sub = df[(df["Temp_K"] == T) &
                 df["gamma_Twin_DMLF_mJm2"].notna() &
                 df["gamma_Twin_Ising_mJm2"].notna()].copy()
        if sub.empty:
            continue
        ...
        for comp, grp in sub.groupby("CompName"):
            x = grp["gamma_Twin_DMLF_mJm2"].to_numpy()
            y = grp["gamma_Twin_Ising_mJm2"].to_numpy()
        ...
        allx = sub["gamma_Twin_DMLF_mJm2"].to_numpy()
        ally = sub["gamma_Twin_Ising_mJm2"].to_numpy()
        ...
        ax.set_xlabel(r"DMLF $\gamma_{\mathrm{Twin}}$ [mJ/m$^2$]")
        ax.set_ylabel(r"Ising (AIM) $\gamma_{\mathrm{Twin}}$ [mJ/m$^2$]")
        ax.set_title(f"Twin boundary energy: DMLF vs Ising (AIM) at {T} K")
...
def main():
    ...
    out_csv = "SFE_DMLF_Ising_Twin_comparison.csv"
